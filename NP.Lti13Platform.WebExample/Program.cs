...
namespace NP.Lti13Platform.WebExample
{
    ...
    public class DataService : IDataService
    {
        ...
        Task<Tool?> ICoreDataService.GetToolAsync(string clientId, CancellationToken cancellationToken)
        {
            return Task.FromResult(Tools.SingleOrDefault(t => t.ClientId == clientId));
        }

        Task<Deployment?> ICoreDataService.GetDeploymentAsync(string deploymentId, CancellationToken cancellationToken)
        {
            return Task.FromResult(Deployments.SingleOrDefault(d => d.Id == deploymentId));
        }

        Task<Context?> ICoreDataService.GetContextAsync(string contextId, CancellationToken cancellationToken)
        {
            return Task.FromResult(Contexts.SingleOrDefault(c => c.Id == contextId));
        }

        Task<User?> ICoreDataService.GetUserAsync(string userId, CancellationToken cancellationToken)
        {
            return Task.FromResult(Users.SingleOrDefault(u => u.Id == userId));
        }

        Task<Membership?> ICoreDataService.GetMembershipAsync(string contextId, string userId, CancellationToken cancellationToken)
        {
            return Task.FromResult(Memberships.SingleOrDefault(m => m.ContextId == contextId && m.UserId == userId));
        }

        Task<IEnumerable<string>> ICoreDataService.GetMentoredUserIdsAsync(string contextId, string userId, CancellationToken cancellationToken)
        {
            return Task.FromResult<IEnumerable<string>>([]);
        }

        Task<ResourceLink?> ICoreDataService.GetResourceLinkAsync(string resourceLinkId, CancellationToken cancellationToken)
        {
            return Task.FromResult(ResourceLinks.SingleOrDefault(r => r.Id == resourceLinkId));
        }

        Task<PartialList<LineItem>> ICoreDataService.GetLineItemsAsync(string deploymentId, string contextId, int pageIndex, int limit, string? resourceId, string? resourceLinkId, string? tag, CancellationToken cancellationToken)
        {
        ...
        async Task<Attempt?> ICoreDataService.GetAttemptAsync(string resourceLinkId, string userId, CancellationToken cancellationToken)
        {
            return await Task.FromResult(Attempts.SingleOrDefault(a => a.ResourceLinkId == resourceLinkId && a.UserId == userId));
        }

        Task<PartialList<Grade>> IAssignmentGradeDataService.GetGradesAsync(string lineItemId, int pageIndex, int limit, string? userId, CancellationToken cancellationToken)
        {
        ...
        Task<Grade?> ICoreDataService.GetGradeAsync(string lineItemId, string userId, CancellationToken cancellationToken)
        {
        ...
        Task<ServiceToken?> ICoreDataService.GetServiceTokenRequestAsync(string toolId, string serviceTokenId, CancellationToken cancellationToken)
        {
            return Task.FromResult(ServiceTokens.FirstOrDefault(x => x.ToolId == toolId && x.Id == serviceTokenId));
        }

        Task ICoreDataService.SaveServiceTokenRequestAsync(ServiceToken serviceToken, CancellationToken cancellationToken)
        {
        ...
        Task<IEnumerable<SecurityKey>> ICoreDataService.GetPublicKeysAsync(CancellationToken cancellationToken)
        {
        ...
        Task<SecurityKey> ICoreDataService.GetPrivateKeyAsync(CancellationToken cancellationToken)
        {
            var rsaProvider = RSA.Create();
        ...
        Task<PartialList<Membership>> INameRoleProvisioningDataService.GetMembershipsAsync(string deploymentId, string contextId, int pageIndex, int limit, string? role, string? resourceLinkId, DateTime? asOfDate, CancellationToken cancellationToken)
        {
        ...
        Task<IEnumerable<User>> INameRoleProvisioningDataService.GetUsersAsync(IEnumerable<string> userIds, DateTime? asOfDate, CancellationToken cancellationToken)
        {
        ...
        Task<string> IDeepLinkingDataService.SaveContentItemAsync(string deploymentId, string? contextId, ContentItem contentItem, CancellationToken cancellationToken)
        {
        ...
        Task<LineItem?> IAssignmentGradeDataService.GetLineItemAsync(string lineItemId, CancellationToken cancellationToken)
        {
            return Task.FromResult(LineItems.SingleOrDefault(x => x.Id == lineItemId));
        }

        Task IAssignmentGradeDataService.DeleteLineItemAsync(string lineItemId, CancellationToken cancellationToken)
        {
