...
public interface IToolSecurityService
{
    /// <summary>
    ...
    /// <returns>A task that represents the asynchronous operation. The task result contains the <see cref="ToolSecurity"/>
    /// object representing the security configuration for the specified client.</returns>
    Task<ToolSecurity> GetToolSecurityAsync(ClientId clientId, Uri baseUrl, CancellationToken cancellationToken = default);
...
internal class DefaultToolSecurityService(ITokenConfigService tokenConfigService, LinkGenerator linkGenerator) : IToolSecurityService
{
    ...
    public async Task<ToolSecurity> GetToolSecurityAsync(ClientId clientId, Uri authenticationBaseUrl, Uri tokenBaseUrl, Uri jwksBaseUrl, CancellationToken cancellationToken = default)
    {
...
public record ToolSecurity(ClientId ClientId, Uri Issuer, Uri GetAuthenticationUrl, Uri PostAuthenticationUrl, Uri TokenUrl, Uri JwksUrl);