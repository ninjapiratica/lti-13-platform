...
public interface ILti13NameRoleProvisioningDataService
{
    ...
    Task<PartialList<(Membership Membership, User User)>> GetMembershipsAsync(DeploymentId deploymentId, string contextId, int pageIndex, int limit, string? role, string? resourceLinkId, DateTime? asOfDate = null, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<UserPermissions>> GetUserPermissionsAsync(DeploymentId deploymentId, string? contextId, IEnumerable<string> userIds, CancellationToken cancellationToken = default);
}
