    ...
    public static IEndpointRouteBuilder UseLti13PlatformAssignmentGradeServices(this IEndpointRouteBuilder endpointRouteBuilder, Func<EndpointsConfig, EndpointsConfig>? configure = null)
    {
        ...
        endpointRouteBuilder.MapGet(config.LineItemsUrl,
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var deployment = await assignmentGradeDataService.GetDeploymentAsync(deploymentId, cancellationToken);
                if (deployment?.ClientId != tool.ClientId)
                {
                    return Results.NotFound();
                }

                var context = await assignmentGradeDataService.GetContextAsync(contextId, cancellationToken);
                if (context == null)
                {
                    return Results.NotFound();
                }

                var lineItemsResponse = await assignmentGradeDataService.GetLineItemsAsync(deploymentId, contextId, pageIndex.Value, limit ?? int.MaxValue, resource_id, resource_link_id, tag, cancellationToken);
        ...
        endpointRouteBuilder.MapPost(config.LineItemsUrl,
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                {
                    var resourceLink = await assignmentGradeDataService.GetResourceLinkAsync(request.ResourceLinkId.GetValueOrDefault(), cancellationToken);
        ...
        endpointRouteBuilder.MapPost($"{config.LineItemUrl}/scores",
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var isNew = false;
                var grade = await assignmentGradeDataService.GetGradeAsync(lineItemId, request.UserId, cancellationToken);
                if (grade == null)
                {
                    isNew = true;
                    grade = new Grade
